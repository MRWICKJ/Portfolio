...
const Team = () => {
  ...
  return (
    <div className="relative mx-auto py-10 w-full max-w-full bg-[#0a0a0a] text-white">
      ...
      <div className="mx-auto px-4 w-full max-w-6xl grid grid-cols-2 gap-y-6 lg:gap-y-12">

        {/* :PICTURES CONTAINER */}
        <div className="col-span-full lg:col-span-1 flex flex-col sm:flex-row justify-center items-center">
          <ul className="mb-4 sm:mb-0 px-4 flex sm:flex-col justify-center items-center space-x-3 sm:space-x-0 sm:space-y-4">
            {people.map((person, index) => (
              <li key={person.lastname} className={`w-16 h-16 rounded-full overflow-hidden filter transition-transform transform ${index !== member ? "grayscale hover:scale-110" : "scale-125"}`}>
          ...
          <div className="rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition duration-300 transform hover:scale-105">
        ...
        <div className="col-span-full lg:col-span-1 mx-auto lg:mx-0 max-w-lg flex flex-col justify-center space-y-4">
          <span className="font-semibold tracking-widest text-sm text-teal-400">{people[member].role}</span>
          <h3 className="text-4xl sm:text-5xl font-bold space-y-2 leading-tight">
            <span className="block">{people[member].firstname}</span>
            <span className="block text-teal-400">{people[member].lastname}</span>
          </h3>
          <p className="py-2 text-gray-300 text-sm sm:text-base">{people[member].description}</p>
          <div className="flex space-x-6">
            {/* Facebook */}
            <a href={people[member].facebookURL} className="text-gray-500 hover:text-blue-500 transition-colors">
            ...
            {/* LinkedIn */}
            <a href={people[member].linkedinURL} className="text-gray-500 hover:text-blue-600 transition-colors">
              <svg className="w-6 h-6" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 30">
